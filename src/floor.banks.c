#pragma bank 0

#include "floor.h"

const FloorBank bank_floor_test  = { BANK_31, &floor_test };
const FloorBank bank_floor_test2 = { BANK_31, &floor_test2 };

const FloorBank bank_floor1 = { BANK_8, &floor1 };
const FloorBank bank_floor2 = { BANK_8, &floor2 };
const FloorBank bank_floor3 = { BANK_8, &floor3 };
const FloorBank bank_floor4 = { BANK_8, &floor4 };
const FloorBank bank_floor5 = { BANK_8, &floor5 };
const FloorBank bank_floor6 = { BANK_8, &floor6 };
const FloorBank bank_floor7 = { BANK_8, &floor7 };
const FloorBank bank_floor8 = { BANK_8, &floor8 };